  ...
  return (
    <div className="space-y-2">
      {npcs.map((npc) => (
        ...
        >
          {/* 剪影头像 */}
          <div className="w-10 h-10 rounded-full bg-(--bg-primary) border border-(--border-color) flex items-center justify-center flex-shrink-0">
            <svg
              viewBox="0 0 24 24"
          ...
          {!npc.dialogueAvailable && (
            <span className="text-xs text-(--text-muted) px-2 py-0.5 bg-(--bg-primary) rounded">
              沉默
            </span>
          )}
        </div>
